...
**Voice commands.** `VoiceTrigger: TriggerSource` using `SFSpeechRecognizer` with `requiresOnDeviceRecognition = true` and a small vocabulary ("clip", "save that", "stop"). The mic is already open for recording; audio is shared via a second `AVCaptureAudioDataOutput` or by tapping the existing one. Key risk is false positives from crowd noise; mitigate with a confirmation haptic and a short cooldown. Alternative if accuracy is poor: a custom keyword classifier via Sound Analysis + Create ML.
